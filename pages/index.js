...
export default function ApolloToolPage() {
  ...
  const [workflowResult, setWorkflowResult] = useState(null); // To store generated workflow JSON for all batches
      ...
      complete: async (results) => {
        try {
          ...
          if (!results.data[0]?.hasOwnProperty(domainColumnName)) {
            throw new Error(
              `CSV must have a column named '${domainColumnName}'.`
            );
          }

          const extractedDomains = results.data
            .map((row) => row[domainColumnName])
            .filter(Boolean);
          ...
          setStatusMessage(
            `Found ${extractedDomains.length} domains. Processing in ${domainBatches.length} batches...`
          );

          const allResults = [];
          for (let i = 0; i < domainBatches.length; i++) {
            setStatusMessage(
              `Processing batch ${i + 1} of ${domainBatches.length}...`
            );
            ...
            const newLinks = {
              ...
              marketing: urlTemplates.marketing.replace(
                /REPLACE_ME/g,
                searchListId
              ),
  ...
  const generateAllWorkflows = async () => {
    if (batchResults.length === 0) {
      setError("No batch results available. Please generate links first.");
      return;
    }

    try {
      const response = await fetch("/api/generate-workflows", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Failed to generate workflows");
      }
  ...
  const fireTheCannons = async () => {
    if (!workflowResult) {
      setError("No workflow JSON available. Please generate workflows first.");
      return;
    }

    try {
      setStatusMessage("Firing the cannons...");
      
      const response = await fetch("http://mikeai.up.railway.app/api/batch-workflows", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Failed to fire the cannons");
      }

      setStatusMessage("Cannons fired successfully! 🎯");
      setError(null);
    } catch (err) {
      setError(`Failed to fire cannons: ${err.message}`);
      setStatusMessage("");
    }
  };
        ...
        <div style={styles.container}>
          <header style={styles.header}>
            ...
            <p style={styles.subtitle}>
              Upload a CSV with up to 100,000+ domains.
            </p>
          </header>
            ...
            {batchResults.length > 0 && (
              <div style={styles.resultsContainer}>
                <h2 style={styles.resultsTitle}>
                  Generated Links ({batchResults.length}{" "}
                  {batchResults.length > 1 ? "Batches" : "Batch"})
                </h2>
                ...
                {/* Generate All Workflows Button */}
                <div style={styles.workflowSection}>
                  <button
                    onClick={generateAllWorkflows}
                    style={styles.workflowButton}
                  >
                    Generate All Workflows JSON
                  </button>

                  {workflowResult && (
                    <div style={styles.workflowResult}>
                      <label style={styles.label}>
                        Generated JSON for All Batches:
                      </label>
                      <textarea
                        readOnly
                      ...
                      {/* Fire the Cannons Button */}
                      <button 
                        onClick={fireTheCannons}
